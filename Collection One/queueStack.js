...
var Queue = function() {
  ...
  this.enqueue = function(enq){
    inbox.push(enq);
  };

  this.dequeue = function(){
    return inbox.storage.shift();
  };
...
var Stack = function() {
  var storage = [];
  var length = 0;
  // All methods have access to storage and length
  // through closure
...
// FIFO
// Using 2 stacks
// Gives us Constant Amortized time complexity
// For dequeue method
var Queue = function(){
  // Accessed through closure scope by
  // Queue methods
  var inbox = new Stack();
  var outbox = new Stack();
  ...
  this.dequeue = function(){
  // Only flip the stack if there is something
  // in the outbox
    if(outbox.size() === 0){
    // "dump" the inbox into the outbox
      while(inbox.size() !== 0){
